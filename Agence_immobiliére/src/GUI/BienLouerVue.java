...
public class BienLouerVue {

	public JFrame frameBienLouerVue;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				...
				/*	BienLouerVue windowBienLouerVue = new BienLouerVue();
					windowBienLouerVue.frame.setVisible(true); */ 
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() {
	    ...
	    List<Bien> list = ps.afficherBienloues();
		for(Bien i:list){
		...
		frameBienLouerVue.getContentPane().add(scrollPane1);
	}
